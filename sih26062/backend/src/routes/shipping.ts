...
const addCargoToManifestSchema = z.object({
  cargoId: z.string().uuid(),
});

shippingRouter.post(
  '/voyages/:voyageId/manifests/addCargo',
  requireAuth,
  requireRole(['ADMIN', 'LOGISTICS']),
  async (req, res) => {
    ...
    const voyageRow = await pool.query(
      'SELECT destination_station_id FROM voyages WHERE id = $1;',
      [voyageId]
    );
    if (voyageRow.rows.length === 0) return res.status(404).json({ error: 'Voyage not found' });

    const destinationStationId = voyageRow.rows[0].destination_station_id as string;
    ...
    try {
      ...
      const manifestIns = await trx.query(
        'INSERT INTO manifests (voyage_id, status, created_by) '
          + 'VALUES ($1,$2,$3) '
          + 'ON CONFLICT (voyage_id) DO NOTHING RETURNING id;',
      ...
      const cargoItems = await trx.query(
        'SELECT ci.id as cargo_item_id, ci.item_id, ci.qty, ci.unit, ci.destination_station_id, ci.unloading_priority, ci.weight_kg, ci.volume_m3, ci.hazard_class, i.item_type '
          + 'FROM cargo_items ci '
          + 'JOIN items i ON i.id = ci.item_id '
          + 'WHERE ci.cargo_id = $1 AND ci.destination_station_id = $2;',
        [input.data.cargoId, destinationStationId]
      );
      ...
      await trx.query(
        'UPDATE cargo SET status = $1 WHERE id = $2;',
        ['MANIFESTED', input.data.cargoId]
      );
...
shippingRouter.post(
  '/manifests/:manifestId/stowage/generate',
  ...
  async (req, res) => {
    ...
    const manifestItems = await pool.query(
      'SELECT mi.id as manifest_item_id, mi.unloading_priority, mi.priority, mi.cargo_item_id, ci.item_id, i.item_type, ci.hazard_class, mi.destination_station_id '
    ...
    if (items.length === 0) return res.status(400).json({ error: 'Manifest has no active items' });
    ...
    const stowageZoneForUnloadIndex = (unloadIndex: number) => {
      // unloadIndex is 1..n (1 = earliest unload)
      if (unloadIndex <= 3) return 'ZONE_A';
      if (unloadIndex <= 6) return 'ZONE_B';
      return 'ZONE_C';
    };
    ...
    try {
      ...
      await trx.query(
        'DELETE FROM stowage_positions WHERE stowage_plan_id IN (SELECT id FROM stowage_plans WHERE manifest_id = $1);',
        [manifestId]
      );
      ...
      const planIns = await trx.query(
        ...
        [manifestId, 'v1-demo', JSON.stringify({
      ...
      for (let idx = 0; idx < n; idx++) {
        ...
        await trx.query(
          'UPDATE manifest_items SET loading_sequence_recommended = $1, loading_sequence_final = $1, unloading_sequence_final = $2 WHERE id = $3;',
...
shippingRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await trx.query(
        'UPDATE cargo SET status = $1 WHERE id IN (SELECT DISTINCT cargo_item.cargo_id FROM manifest_items mi '
          + 'JOIN cargo_items cargo_item ON cargo_item.id = mi.cargo_item_id WHERE mi.manifest_id = $2);',
        ['LOADED', manifestId]
      );

      // Insert cargo status history
      const histories = await trx.query(
        'SELECT DISTINCT c.id as cargo_id FROM manifest_items mi '
          + 'JOIN cargo_items ci ON ci.id = mi.cargo_item_id '
      ...
      for (const h of histories.rows) {
        await trx.query(
          ...
          [h.cargo_id, 'MANIFESTED', 'LOADED', actorUserId, 'Cargo loaded']
        );
      }
...
shippingRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await trx.query(
        'UPDATE voyages SET status = $1, departure_date = COALESCE(departure_date, now()::date) WHERE id = $2;',
        ['IN_TRANSIT', voyageId]
      );

      await trx.query(
        'UPDATE cargo SET status = $1 WHERE id IN (SELECT DISTINCT ci.cargo_id FROM manifests m '
      ...
      await trx.query(
        'INSERT INTO voyage_status_history (voyage_id, from_status, to_status, changed_by, notes) '
          + 'SELECT $1, $2, $3, $4, $5 WHERE EXISTS (SELECT 1 FROM voyages WHERE id = $1);',
        [voyageId, 'PLANNED', 'IN_TRANSIT', actorUserId, 'Voyage departed']
      );
...
shippingRouter.post(
  '/voyages/:voyageId/receive',
  requireAuth,
  requireRole(['ADMIN', 'LOGISTICS']),
  async (req, res) => {
    const { voyageId } = req.params;
    const actorUserId = (req as AuthedRequest).user!.userId;
    const { receivedAt } = z.object({ receivedAt: z.coerce.date().optional() }).safeParse(req.body)
      ? z.object({ receivedAt: z.coerce.date().optional() }).parse(req.body)
      : { receivedAt: undefined as unknown as Date | undefined };
    ...
    try {
      ...
      await trx.query(
        'UPDATE voyages SET status = $1, actual_arrival = COALESCE($2::date, actual_arrival) WHERE id = $3;',
        ['ARRIVED', receivedAt ? receivedAt.toISOString().slice(0, 10) : null, voyageId]
      );

      const manifestItems = await trx.query(
        'SELECT mi.cargo_item_id, ci.cargo_id, mi.destination_station_id, ci.qty, ci.unit '
          + 'FROM manifests m '
          + 'JOIN manifest_items mi ON mi.manifest_id = m.id '
          + 'JOIN cargo_items ci ON ci.id = mi.cargo_item_id '
          + 'WHERE m.voyage_id = $1;',
      ...
      for (const mi of manifestItems.rows) {
        ...
        await trx.query(
          'INSERT INTO inventory (station_id, item_id, quantity, unit, average_daily_consumption, minimum_stock, safety_stock, last_updated) '
            ...
            + 'SET quantity = inventory.quantity + EXCLUDED.quantity, unit = EXCLUDED.unit, last_updated = now();',
          [mi.destination_station_id, qty, mi.unit, mi.cargo_item_id]
        );
      ...
      await trx.query(
        'UPDATE manifests SET status = $1 WHERE voyage_id = $2;',
        ['RECEIVED', voyageId]
      );
